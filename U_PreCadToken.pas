...
begin
  with dm do
    begin
      ...
      SqlAux2.Params[00].AsString  :=  '2241';//'1';//
      SqlAux2.Open;
                ...
                begin
                  ...
                  Else
                    Begin
                      SqlAux3.Close;
                      SqlAux3.SQL.Clear;
                      SqlAux3.SQL.Add('select min(tbsdxect_num),tbsdxect_dv from tbsdx_ect where (tbsdxect_prod = ''2241'') and (tbsdx_use is null) group by tbsdxect_num,tbsdxect_dv order by tbsdxect_num limit 1');
                      SqlAux3.Open;
...
begin
  with dm do
    begin
      ...
      SqlAux2.SQL.Clear;
      SqlAux2.SQL.Add('select count(tbsdxect_num) from tbsdx_ect where (tbsdx_use is null) and (tbsdxect_prod = :prod)');
      SqlAux2.Params[00].AsString  :=  '2240';//'0';//
      SqlAux2.Open;
      if (SqlAux2.Fields[0].Value > 0) and (SqlAux2.Fields[0].Value <= 200) then
        Application.MessageBox(PChar('Solicite aod Correiod Novo Range'+chr(10)+'Contém 200 ou menos'),'ADS',ID_OK)
        ...
        begin
          ...
          repeat
            Begin
              SqlAux3.Close;
              SqlAux3.SQL.Clear;
              SqlAux3.SQL.Add('select min(tbsdxect_num),tbsdxect_dv from tbsdx_ect where (tbsdxect_prod = ''2240'') and (tbsdx_use is null) group by tbsdxect_num,tbsdxect_dv order by tbsdxect_num limit 1');
              SqlAux3.Open;
              numobj:= SqlAux3.Fields[0].AsString+SqlAux3.Fields[1].AsString;
              SqlTbBradDeptos.Close;
              SqlTbBradDeptos.SQL.Clear;
